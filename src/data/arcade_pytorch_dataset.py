...
class ARCADEStenosisDataset(Dataset):
    ...
    def __init__(self, annotations_file, images_dir, stenosis_category_id=26):
    ...
    def __getitem__(self, index):
        ...
        mask = self._create_mask(height, width, annotations)
...
def visualizar_batch(batch):
    ...
    for i in range(n):
        ...
        overlay[lesion] = 0.45 * overlay[lesion] + 0.55 * np.array([1.0, 0.0, 0.0])
        ...
        axes[i, 2].set_title("Imagen + máscara")
...
if __name__ == "__main__":
    ...
    print("  image min/max:", float(sample["image"].min()), float(sample["image"].max()))
    print("  valores máscara:", torch.unique(sample["mask"]).tolist())
    ...
    print("  image IDs:", batch["image_id"].tolist())
    print("  archivos:", batch["file_name"])

    # --------------------------------------------
    # PORCENTAJE DE PÍXELES DE LESIÓN
    # --------------------------------------------

    lesion_percentage = batch["mask"].sum(dim=(1, 2, 3)) / batch["mask"].numel() * 100

    print()
    print("Píxeles de lesión por muestra (% del batch):")

    for i, value in enumerate(lesion_percentage):
        print(f"  {batch['file_name'][i]}: {float(value):.4f}%")
    ...
    print()
    print("Mostrando el primer batch...")
