...
TRAIN_JSON = PROJECT_ROOT / "arcade" / "stenosis" / "train" / "annotations" / "train.json"
...
stenosis_per_image = (
    stenosis_df.groupBy("image_id").count().withColumnRenamed("count", "num_stenoses")
)
...
image_stenosis_df = images_df.join(
    stenosis_per_image, images_df.id == stenosis_per_image.image_id, "left"
).select(
...
lesion_features_df = (
    annotations_df.filter(col("category_id") == 26)
    .select(col("id").alias("annotation_id"), col("image_id"), col("area"), col("bbox"))
...
lesion_features_df.select(
    "annotation_id", "image_id", "area", "x", "y", "lesion_width", "lesion_height"
).show(10)

lesion_features_df = lesion_features_df.withColumn(
    "aspect_ratio",
    when(col("lesion_height") > 0, col("lesion_width") / col("lesion_height")).otherwise(None),
)

lesion_features_df = lesion_features_df.withColumn(
    "bbox_area", col("lesion_width") * col("lesion_height")
)
...
lesion_features_df = lesion_features_df.join(
    images_df.select(
        col("id").alias("img_id"),
...
lesion_features_df = lesion_features_df.withColumn(
    "relative_area", col("area") / (col("image_width") * col("image_height"))
)
...
lesion_features_df.select(
    "area", "bbox_area", "lesion_width", "lesion_height", "aspect_ratio", "relative_area"
).describe().show()

lesion_features_df.select("area", "lesion_width", "lesion_height", "aspect_ratio").summary(
...
lesion_features_df.write.mode("overwrite").parquet(str(OUTPUT_DIR))
