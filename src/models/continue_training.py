...
class ArcadeStenosisDataset(Dataset):
    def __init__(self, root, split, image_size=256):
        ...
        with open(ann_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        self.images_info = [x for x in data["images"]
...
class UNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.enc1=DoubleConv(1,32); self.enc2=DoubleConv(32,64); self.enc3=DoubleConv(64,128)
        self.pool=nn.MaxPool2d(2); self.bottleneck=DoubleConv(128,256)
...
def metrics(logits, targets):
    ...
    fp=(pred*(1-targets)).sum((1,2,3)); fn=((1-pred)*targets).sum((1,2,3))
...
def validate(model,loader,device,criterion):
    model.eval(); losses=[]; vals=[]
    for x,y,_ in loader:
        x,y=x.to(device),y.to(device); z=model(x); losses.append(criterion(z,y).item())
...
def save_predictions(model,dataset,device,epoch,n=2):
    ...
    for r,i in enumerate(idxs):
        x,y,name=dataset[i]; z=model(x.unsqueeze(0).to(device))
        p=torch.sigmoid(z)[0,0].cpu().numpy(); pred=(p>=THRESHOLD)
        ax[r,0].imshow(x[0],cmap="gray"); ax[r,0].set_title(f"Original\n{name}")
        ax[r,1].imshow(y[0],cmap="gray"); ax[r,1].set_title("Ground Truth")
        ax[r,2].imshow(p,cmap="gray",vmin=0,vmax=1); ax[r,2].set_title("Probabilidad")
        ax[r,3].imshow(pred,cmap="gray"); ax[r,3].set_title("Predicción")
        for c in range(4): ax[r,c].axis("off")
    plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR,f"predicciones_epoch_{epoch:02d}.png"),dpi=150); plt.close()

device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("="*72); print(" CONTINUACIÓN DEL ENTRENAMIENTO U-NET - ARCADE STENOSIS"); print("="*72)
...
best_val_dice=-1.0
history={"epoch":[],"train_loss":[],"val_loss":[],"train_dice":[],"val_dice":[],"val_iou":[],"val_precision":[],"val_recall":[]}

for epoch in range(START_EPOCH,END_EPOCH+1):
    t=time.time(); model.train(); losses=[]; dices=[]
    print("\n"+"="*72); print(f"ÉPOCA {epoch}/{END_EPOCH}"); print("="*72)
    for bi,(x,y,_) in enumerate(train_dl,1):
        x,y=x.to(device),y.to(device); optimizer.zero_grad()
        z=model(x); loss=criterion(z,y); loss.backward(); optimizer.step()
        losses.append(loss.item()); dices.append(metrics(z.detach(),y)[0])
        if bi==1 or bi%100==0 or bi==len(train_dl):
            print(f"  batch {bi:3d}/{len(train_dl)} | loss {loss.item():.4f}")
    train_loss=np.mean(losses); train_dice=np.mean(dices)
    val_loss,val_dice,val_iou,val_precision,val_recall=validate(model,val_dl,device,criterion)
    mins=(time.time()-t)/60
    ...
    history["train_dice"].append(train_dice); history["val_dice"].append(val_dice); history["val_iou"].append(val_iou)
    history["val_precision"].append(val_precision); history["val_recall"].append(val_recall)
    with open(os.path.join(OUT_DIR,"historial_continuado.json"),"w",encoding="utf-8") as f: json.dump(history,f,indent=2)
    if val_dice>best_val_dice:
        best_val_dice=val_dice
        torch.save({"model_state_dict":model.state_dict(),"epoch":epoch,"val_dice":val_dice,
...
print("\n"+"="*72); print(" ENTRENAMIENTO CONTINUADO TERMINADO"); print("="*72)
